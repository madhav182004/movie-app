  ...
  const fetchAllMovies = async () => {
    try {
      ...
      if (search) {
        ...
        API_URL = `https://api.themoviedb.org/3/movie/top_rated?api_key=${Api_key}&language=en-US&page=${page}`
      }
    ...
    <>
      <div>
        <h2 className="text-2xl font-bold text-center md:text-left pb-4">
          {search ? 'Search Result for ' + search : 'Top Rated Movies'}
        </h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="mx-4 my-8 grid grid-cols-2 md:grid-cols-4 gap-5">
          {!allMovies
            ? 'Loading...'
            : allMovies.length === 0
            ? 'No movies found'
